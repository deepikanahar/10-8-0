...
@implementation CustomColoredNumberXAxisWF
...
@implementation CustomColoredWFSeries
...
@implementation I2IWFColoredPlot {
...
-(void)renderChart:(UIView *)hostView identifier:(NSString *)identifier {
    ...
    yAxis = [CustomColoredNumberXAxisWF new];
    ...
    if ([[arrStops firstObject] intValue] > 0) {
        for (id object in arrStops) {
            if ([object intValue] < dataForPlot.count - 1) {
                ...
                vLine = [SChartAnnotation verticalLineAtPosition:@([object intValue]+0.5) withXAxis:wfChart.xAxis andYAxis:wfChart.yAxis withWidth:0.25f withColor:[UIColor lightGrayColor]];
...
-(SChartSeries *)sChart:(ShinobiChart *)chart seriesAtIndex:(NSInteger)index {
    CustomColoredWFSeries *candleStickSeries = [[CustomColoredWFSeries alloc] init];
...
-(void)sChart:(ShinobiChart *)chart alterDataPointLabel:(SChartDataPointLabel *)label forDataPoint:(SChartDataPoint *)dataPoint inSeries:(CustomColoredWFSeries *)series {
