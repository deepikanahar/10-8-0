...
#import "I2IWFColoredLabels.h"

@implementation I2IWFColoredLabels
...
-(id)initViewer:(ViewerDataModel*)_viewerDataModel withCommanderDelegate:(id<MSICommanderDelegate>)_commander withProps:(NSString*)_props{
    ...
    if(self){
        ...
        wfGraph = [[I2IWFColoredPlotLabels alloc] init];
...
-(void)readConstants {
    ...
    // Always expect this metric to be the number of controls
...
-(void)readData {
    ...
    wfGraph.secondaryLabels = [[NSMutableArray alloc] init];
    
    int rowID = 6 + 8 * intControls + 2 * intBars;
    ...
    for (int i = 0; i < intBars; i++) {
        ...
        //[wfGraph.dataForPlot addObject:answer storeKey:strVariable];
        
        [wfGraph.arrXLabels addObject:cell.headerValue];
    }
    
    //Loop through the 2nd data labels to be added as percentage of revenue to the graph
    rowID = 6 + 8 * intControls + intBars;
    NSMutableArray *row = [self.modelData arrayWithHeaderValueOfWholeRowByAxisType:ROW_AXIS andRowIndex:rowID];
    MSIMetricValue *metricProperties = [row objectAtIndex:1];
    MSIPropertyGroup *propertyGroup = metricProperties.format;
    [wfGraph.colors addObject:[self colorConvertor:[propertyGroup propertyByPropertySetID:FormattingFont propertyID:FontFormattingColor]]];
    for (int i = 0; i < intBars; i++) {
        NSString *val = [[[self.modelData arrayWithHeaderValueOfWholeRowByAxisType:ROW_AXIS andRowIndex:rowID] objectAtIndex:1] rawValue];
        
        if(val.doubleValue < 0) {
            [wfGraph.secondaryLabels addObject:[NSString stringWithFormat:@"(%.1f)", [val doubleValue]*-1]];
        }
        else {
            [wfGraph.secondaryLabels addObject:[NSString stringWithFormat:@"%.1f", [val doubleValue]]];
        }
...
-(void)textFieldDidEndEditing:(UITextField *)textField {
    ...
    if (isValidInput == YES) {
        ...
        [accessoryTf selectAll:accessoryTf.text];
    }
}
