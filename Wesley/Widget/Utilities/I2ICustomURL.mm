...
-(void)readData {
    ...
    //DN - header -> Uncomment once MSTR changes are done
    //customURL = [[[self.dataModel arrayWithHeaderValueOfWholeRowByAxisType:COLUMN_AXIS andRowIndex:0] objectAtIndex:0] headerValue];
    ...
    if ([linkViewType isEqualToString:viewTypeImage]) {
        ...
        metricValue = [metricHeader.elements objectAtIndex:0];
        //linkViewDisplay = metricValue.rawValue;
        MSIHeaderValue *header = [[self.dataModel arrayWithHeaderValueOfWholeRowByAxisType:COLUMN_AXIS andRowIndex:2] objectAtIndex:0];
        linkViewDisplay = header.headerValue;
        ...
        UIColor *bgColor = [self colorConvertor:[propertyGroup propertyByPropertySetID:FormattingFont
        ...
        linkLabel.backgroundColor = bgColor;
        linkLabel.layer.masksToBounds = YES;
        linkLabel.layer.cornerRadius = 8.0;
