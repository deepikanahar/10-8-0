...
#import "I2IWFExtended.h"

@implementation I2IWFExtended
...
-(id)initViewer:(ViewerDataModel*)_viewerDataModel withCommanderDelegate:(id<MSICommanderDelegate>)_commander withProps:(NSString*)_props{
    ...
    if(self){
        ...
        wfGraph = [[I2IWaterFallPlot alloc] init];
...
-(void)recreateWidget {
    ...
    // CGrect offset to account for graph title position
    NSArray *arrPosition = [[NSArray alloc] initWithArray:[chartPosition componentsSeparatedByString:@","]];
    CGRect frame = CGRectMake([[arrPosition objectAtIndex:0] floatValue], [[arrPosition objectAtIndex:1] floatValue], [[arrPosition objectAtIndex:2] floatValue], [[arrPosition objectAtIndex:3] floatValue]);
    hostView = [[UIView alloc] initWithFrame:frame];
    ...
    for (I2IDynamicLabel *i2iLabel in labels) {
        arrPosition = [[NSArray alloc] initWithArray:[i2iLabel.position componentsSeparatedByString:@","]];
        frame = CGRectMake([[arrPosition objectAtIndex:0] floatValue], [[arrPosition objectAtIndex:1] floatValue], [[arrPosition objectAtIndex:2] floatValue], [[arrPosition objectAtIndex:3] floatValue]);
        // -999<i> is tag for Dynamic label.
        NSInteger intTag = [[NSString stringWithFormat:@"%@%d",@"-999",i] integerValue];
        [self addSubview:[i2iLabel initializeLabel:frame withTag:intTag]];
        i++;
    }
    // Evaluates the formulae and updates the value of dynamic labels & texts.
    [self updateLabels];
    [self updateTexts];
...
-(void)readConstants {
    ...
    //  Always expect third metric to be the graph ID
    ...
    //  Always expect fourth metric to be the number of bars
    ...
    //  Always expect fifth metric to be the graph position
    ...
    //  Always expect sixth metric to be the bars where the waterfall graph would restart
    ...
    //  Always expect seventh metric to be the number of controls
    metricValue = [metricHeader.elements objectAtIndex:intBars + 6];
    ...
    if (intLabels > 0) {
        for (int i = 0; i < intLabels; i++) {
            rowID = 8 + (8 * intControls) + intBars + (2 * i);
...
#pragma mark Supporting Metrics
    //  Loop through all the base metrics and add to the key-value pair to metrics dictionary.
    ...
    //  Writes tab/panel identifier to plist file.
...
-(void)readData {
    ...
    for (int i = 0; i < intBars; i++){
        MSIHeaderValue *cell = [[self.modelData arrayWithHeaderValueOfWholeRowByAxisType:ROW_AXIS andRowIndex:i+6] objectAtIndex:0];
...
#pragma mark Create i2i Controls
        ...
        if ([i2iControl.uid isEqualToString:[[@(slider.tag) stringValue] stringByReplacingOccurrencesOfString:@"999" withString:@"Slider"]]) {
            ...
            if ([i2iControl.suffix isEqualToString:@"%"]) {
                if ([i2iControl.step isEqualToString:@"1"]) {
                    ...
                    if(strSliderVal > i2iControl.max)
                        strSliderVal = i2iControl.max;
                }
                else {
        ...
        if ([i2iControl.uid isEqualToString:[[@(accessoryTf.tag) stringValue] stringByReplacingOccurrencesOfString:@"919" withString:@"Input"]]) {//919 is tag for Input values
            if ([self validateTextField:accessoryTf.text]==YES) {
                ...
                if (fUpdatedString >= [i2iControl.min doubleValue] && fUpdatedString <= [i2iControl.max doubleValue]) {
                    ...
                    NSString *percentStr = @"%";
                    if ([i2iControl.suffix isEqualToString:@"%"]) {
                        [self showAlertWithMessage:[NSString stringWithFormat:@"Please enter value within %d%@ to %d%@",[i2iControl.min intValue]*100,percentStr,[i2iControl.max intValue]*100,percentStr]];//Message to validate textbox
                    }
                    else{
                        
                        [self showAlertWithMessage:[NSString stringWithFormat:@"Please enter value within %@ to %@",i2iControl.min,i2iControl.max]];//Message to validate textbox
                    }
                ...
                [self showAlertWithMessage:[NSString stringWithFormat:@"Please enter valid input"]];//Message to validate textbox
                double fDefaultValue = [[[PlistData getValue] valueForKey:i2iControl.uid] doubleValue];
                accessoryTf.text=[self setNumberFormat:[NSString stringWithFormat:@"%lf",fDefaultValue] withFormatCategory:i2iControl.category withFormat:i2iControl.format];
                isValidInput = NO;
                [accessoryTf resignFirstResponder];
...
#pragma mark Converts BGR to UIColor
...
#pragma mark Updates Dynamic Components
-(void)updateLabels {
    for (int i = 0; i < intLabels; i++) {
...
-(void)updateTexts {
    ...
    if (intControls > 0) {
        rowID = 8 + (8 * intControls) + (2 * intLabels) + intBars;
    }
    ...
    if(intTexts > 0) {
        ...
        for (int i = 0; i < intTexts; i++) {
            ...
            position = [[NSString alloc]initWithFormat:@"%@",[[metricHeader.elements objectAtIndex:indexOfText++] rawValue]];
            NSArray *arrPosition = [[NSArray alloc] initWithArray:[position componentsSeparatedByString:@","]];
            CGRect frameRect = CGRectMake([[arrPosition objectAtIndex:0] floatValue], [[arrPosition objectAtIndex:1] floatValue], [[arrPosition objectAtIndex:2] floatValue], [[arrPosition objectAtIndex:3] floatValue]);
            ...
            if (intNoFormulae > 0) {
                ...
                for (int j = 0; j < intNoFormulae; j++) {
                    ...
                    // Number Format
                    NSString *strFormatCategory = [propertyGroup propertyByPropertySetID:FormattingNumber propertyID:NumberFormattingCategory];
                    ...
                    NSString *str = [self setNumberFormat:tempValue withFormatCategory:strFormatCategory withFormat:strNumberFormat];
